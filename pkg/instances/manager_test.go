...
func TestManager_ListRuntimes(t *testing.T) {
	...
	t.Run("returns all registered runtimes", func(t *testing.T) {
		t.Parallel()
		...
		// Initially no runtimes should be registered
		runtimes := manager.ListRuntimes()
		if len(runtimes) != 0 {
			t.Errorf("Expected 0 runtimes initially, got %d", len(runtimes))
		}
		...
		// Should now have 1 runtime
		runtimes = manager.ListRuntimes()
		if len(runtimes) != 1 {
			t.Errorf("Expected 1 runtime, got %d", len(runtimes))
		}

		// Verify it's the fake runtime
		if len(runtimes) > 0 && runtimes[0] != "fake" {
			t.Errorf("Expected runtime 'fake', got %s", runtimes[0])
		}
	})
}
