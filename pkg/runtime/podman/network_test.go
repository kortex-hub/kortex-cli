...
func TestSetupFirewallRules(t *testing.T) {
	...
	t.Run("uses podman machine resolv.conf fallback on WSL2 when getent fails", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("uses podman machine resolv.conf fallback on WSL2 when getent returns 127.0.0.1", func(t *testing.T) {
		t.Parallel()
		...
		fakeExec.OutputFunc = func(ctx context.Context, args ...string) ([]byte, error) {
			...
			if len(args) >= 4 && args[0] == "machine" && args[1] == "ssh" {
				return []byte("nameserver 172.20.0.1\nsearch localdomain\n"), nil
			}
			return []byte{}, nil
		}
...
func TestInjectWSLHostEntry(t *testing.T) {
	...
	t.Run("adds /etc/hosts entry with nameserver IP from podman machine", func(t *testing.T) {
		...
		fakeExec.OutputFunc = func(ctx context.Context, args ...string) ([]byte, error) {
			if len(args) >= 4 && args[0] == "machine" && args[1] == "ssh" {
				return []byte("nameserver 10.255.0.1\n"), nil
			}
			return []byte{}, nil
		}
	...
	t.Run("returns error when no nameserver in resolv.conf", func(t *testing.T) {
		t.Parallel()

		fakeExec := exec.NewFake()
		fakeExec.OutputFunc = func(ctx context.Context, args ...string) ([]byte, error) {
			if len(args) >= 4 && args[0] == "machine" && args[1] == "ssh" {
				return []byte("search localdomain\n# no nameserver\n"), nil
			}
			return []byte{}, nil
		}
		...
		err := rt.injectWSLHostEntry(context.Background(), "test-container")
		if err == nil {
			t.Fatal("expected error when no nameserver in resolv.conf, got nil")
		}
	})
}
...
func TestResolveWSLHostIP(t *testing.T) {
	...
	t.Run("extracts first nameserver via podman machine ssh", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("returns empty when no nameserver line", func(t *testing.T) {
		t.Parallel()

		fakeExec := exec.NewFake()
		fakeExec.OutputFunc = func(ctx context.Context, args ...string) ([]byte, error) {
			if len(args) >= 4 && args[0] == "machine" && args[1] == "ssh" {
				return []byte("search example.com\n"), nil
			}
			return []byte{}, nil
		}
